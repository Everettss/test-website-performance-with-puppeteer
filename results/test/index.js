...
const test = async (runs, emulateConditions, filePath) => {
    ...
    for (let i = 0; i < runs; i++) {
        ...
        // first enter test
        res = [...res, ...convertResultsToArr(await testPage(page, client), 'firstEnter')];
        ...
        res = [...res, ...convertResultsToArr(await testPage(page, client), 'swAndCache')];
        ...
        res = [...res, ...convertResultsToArr(await testPage(page, client), 'cache')];
        bar.tick();
        await browser.close();
